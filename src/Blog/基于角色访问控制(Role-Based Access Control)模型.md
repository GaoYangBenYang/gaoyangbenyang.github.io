...
### RBAC权限模型组成
  ...
  RBAC模型是RBAC机制的基本模型，又称为RBAC0(Core RBAC)，除此之外，还有一些在此基础上演化出来的模型，如角色分层模型RBAC1（Hierarchal RBAC）、角色限制模型RBAC2（Constraint RBAC）和统一模型RBAC3（Combines RBAC）。
...
![RBAC权限体系精简设计](/assets/blog/基于角色访问控制模型/RBAC权限体系精简设计.png)
